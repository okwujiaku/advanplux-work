...
function Dashboard() {
  ...
  const { signOut, refetchWalletAndDeposits, isAdminViewAsMode, clearAdminViewAs, users, currentUserId } = useApp()
  const location = useLocation()
  const currentUser = users?.find((u) => u.id === currentUserId)
  const viewAsEmail = currentUser?.email ?? 'member'
      ...
      {isAdminViewAsMode && (
        <div className="bg-amber-500 text-amber-950 px-4 py-2 flex flex-wrap items-center justify-center gap-3 text-sm font-medium">
          ...
          <button
            type="button"
            onClick={exitViewAs}
            className="px-3 py-1 rounded bg-amber-900 text-amber-100 hover:bg-amber-800"
          >
            Exit to home
          </button>
        </div>
