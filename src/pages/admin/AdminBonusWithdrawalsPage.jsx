  ...
  return (
    <section className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-4">
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        <select value={form.memberId} onChange={(e) => setForm((p) => ({ ...p, memberId: e.target.value }))} className="px-4 py-3 border border-gray-300 rounded-lg">{displayMembers.map((member) => <option key={member.id} value={member.id}>{getMemberDisplay(member)}</option>)}</select>
